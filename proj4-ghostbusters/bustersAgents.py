...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        centers = [belief.argMax() for belief in livingGhostPositionDistributions]
        dis_centers = [(self.distancer.getDistance(pacmanPosition, pos), pos) for pos in centers]
        targetPos = min(dis_centers)[1]

        dis_actions = [(self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, a), targetPos), a)
                       for a in legal]
        action = min(dis_actions)[1]
        return action
